...
int g_number_of_k_paths_iterations = 0;
int g_current_iteration_number = 0;
	...
	void CalculateLabelCost(int vehicle_id)
	{
		...
		for (std::map<int, int>::iterator it = passenger_service_state.begin(); it != passenger_service_state.end(); ++it)
		{
			if (it->second == 2)  // complete
			{
				...
				// Tony: I don't comment the basic profit of passengers
				...
				//Tony: I comment the waiting time of passengers
		...
		//Tony: I comment the waiting time of vehicles
...
void g_output_optimization_result()
{
	...
	for (int j = 1; j <= g_number_of_vehicles; j++)
	{
		...
		fprintf(g_pFile_OutputAgentLog, ",");
		fprintf(g_pFile_OutputAgentLog, "%d", g_current_iteration_number);
		fprintf(g_pFile_OutputAgentLog, ",");
		...
		for (int i = 0; i < g_ending_state_vector[j].m_VSStateVector[0].m_visit_node_sequence.size(); i++)
		{
			/*if (g_node_passenger_id[g_ending_state_vector[j].m_VSStateVector[0].m_visit_node_sequence[i]] == 0)
				continue;
	...
	for (int j = 1; j <= g_number_of_vehicles; j++)
	{
		...
		fprintf(g_pFile_OutputAgentPathArcLog, ",");
		fprintf(g_pFile_OutputAgentPathArcLog, "%d", g_current_iteration_number);
		fprintf(g_pFile_OutputAgentPathArcLog, ",");
...
void g_Output_GAMS_Set_needs()
{
	...
	fprintf(g_pFileGAMSinputSetLog, "Set k /1*%d/;\n", k_max);
...
void g_Output_GAMS_Parameter_needs()
{
	...
	for (int j = 1; j <= g_number_of_vehicles; j++)
	{
		//fprintf(g_pFileGAMSinputParameterLog, "%d.%d %.2f\n", j, g_current_iteration_number, g_vehicle_path_cost[j]);
	...
	for (int j = 1; j <= g_number_of_vehicles; j++)
	{
		...
		if (served_passenger_id.size() == 0)
		{
			fprintf(g_pFileGAMSinputParaIncidVehPathPaxCsv, "%d.%d.%d %d\n", j, g_current_iteration_number, 1, 1);
		}
		else
		{
			for (int i = 0; i < served_passenger_id.size(); i++)
			{
				//fprintf(g_pFileGAMSinputParameterLog, "%d.%d.%d %d\n", j, g_current_iteration_number, served_passenger_id[i],1);
				fprintf(g_pFileGAMSinputParaIncidVehPathPaxCsv, "%d.%d.%d %d\n", j, g_current_iteration_number, served_passenger_id[i], 1);
			}
		}
	...
	for (int j = 1; j <= g_number_of_vehicles; j++)
	{
		...
		//fprintf(g_pFileGAMSinputParameterLog, "%d.%d.%d.%d.%d.%d %d\n", j, g_current_iteration_number, arc_origin, arc_origin_next, arc_dep, arc_dep_next, 1);
		...
		for (int i = 0; i < g_ending_state_vector[j].m_VSStateVector[0].m_visit_node_sequence.size() - 1; i++)
		{
			...
			//fprintf(g_pFileGAMSinputParameterLog, "%d.%d.%d.%d.%d.%d %d\n", j, g_current_iteration_number, arc_i, arc_j, arc_t, arc_s, 1);
			fprintf(g_pFileGAMSinputParaIncidVehPathArcCsv, "%d.%d.%d.%d.%d.%d %d\n", j, g_current_iteration_number, arc_i, arc_j, arc_t, arc_s, 1);
		}
	}
	//fprintf(g_pFileGAMSinputParameterLog, "/;\n");
...
void g_ReadGamsMargData()
{
	...
	CCSVParser parser;
	if (parser.OpenCSVFile("Output_GAMS_arc_cap_marg.csv", true))
	{
		while (parser.ReadRecord())  // if this line contains [] mark, then we will also read field headers.
		{
	...
	if (parser.OpenCSVFile("Output_GAMS_passenger_pickup_marg.csv", true))
	{
		while (parser.ReadRecord())  // if this line contains [] mark, then we will also read field headers.
		{
	...
	if (parser.OpenCSVFile("Output_GAMS_vehicle_weight_marg.csv", true))
	{
		while (parser.ReadRecord())  // if this line contains [] mark, then we will also read field headers.
		{
...
void g_Read_Iteration_Num_from_Py()
{
	CCSVParser parser;
	if (parser.OpenCSVFile("Py_output_num_VRP.csv", true))
	{
		while (parser.ReadRecord())  // if this line contains [] mark, then we will also read field headers.
		{
			if (parser.GetValueByFieldName("Current_iteration_number", g_current_iteration_number) == false)
				continue;

			if (g_current_iteration_number <= 0)
			{
				cout << "Current_iteration_number " << g_current_iteration_number << " is out of range" << endl;
				g_ProgramStop();
			}

			if (parser.GetValueByFieldName("Total_number_of_iterations", g_number_of_k_paths_iterations) == false)
				continue;

			if (g_number_of_k_paths_iterations <= 0)
			{
				cout << "Total_number_of_iterations " << g_number_of_k_paths_iterations << " is out of range" << endl;
				g_ProgramStop();
			}
...
int _tmain(int argc, TCHAR* argv[], TCHAR* envp[])
{
	...
	g_pFileGAMSinputSetCsv = fopen("Sub_GAMS_input_set.csv", "w");
	if (g_pFileGAMSinputSetCsv == NULL)
	{
		cout << "File Sub_GAMS_input_set.csv cannot be opened." << endl;
		g_ProgramStop();
	}

	//output csv file for GAMS input parameter: VehPathCost
	g_pFileGAMSinputParaVehPathCostCsv = fopen("Sub_GAMS_input_veh_path_cost.csv", "w");
	if (g_pFileGAMSinputParaVehPathCostCsv == NULL)
	{
		cout << "File Sub_GAMS_input_veh_path_cost.csv cannot be opened." << endl;
		g_ProgramStop();
	}

	//output csv file for GAMS input parameter:VehPathSelection
	g_pFileGAMSinputParaVehPathSelectionCsv = fopen("Sub_GAMS_input_veh_path_selection.csv", "w");
	if (g_pFileGAMSinputParaVehPathSelectionCsv == NULL)
	{
		cout << "File Sub_GAMS_input_veh_path_selection.csv cannot be opened." << endl;
		g_ProgramStop();
	}

	//output csv file for GAMS input parameter: IncidVehPathPax
	g_pFileGAMSinputParaIncidVehPathPaxCsv = fopen("Sub_GAMS_input_incid_veh_path_pax.csv", "w");
	if (g_pFileGAMSinputParaIncidVehPathPaxCsv == NULL)
	{
		cout << "File Sub_GAMS_input_incid_veh_path_pax.csv cannot be opened." << endl;
		g_ProgramStop();
	}

	//output csv file for GAMS input parameter: IncidVehPathArc
	g_pFileGAMSinputParaIncidVehPathArcCsv = fopen("Sub_GAMS_input_incid_veh_path_arc.csv", "w");
	if (g_pFileGAMSinputParaIncidVehPathArcCsv == NULL)
	{
		cout << "File Sub_GAMS_input_incid_veh_path_arc.csv cannot be opened." << endl;
		g_ProgramStop();
	}

	//output csv file for GAMS input parameter: ArcCap
	g_pFileGAMSinputParaArcCapCsv = fopen("Sub_GAMS_input_arc_cap.csv", "w");
	if (g_pFileGAMSinputParaArcCapCsv == NULL)
	{
		cout << "File Sub_GAMS_input_arc_cap.csv cannot be opened." << endl;
		g_ProgramStop();
	}
	...
	g_Read_Iteration_Num_from_Py();

	//stringstream ss;
	//ss << g_current_iteration_number;
	//string name = "output_agent_" + ss.str() + ".csv";
	//cout << name << endl;
	//g_pFile_OutputAgentLog = fopen("name.csv", "w");

	g_pFile_OutputAgentLog = fopen("output_agent_internal.csv", "w");
	if (g_pFile_OutputAgentLog == NULL)
	{
		cout << "File output_agent_internal.csv cannot be opened." << endl;
		g_ProgramStop();
	}

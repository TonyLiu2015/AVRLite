...
#define _MAX_NUMBER_OF_DEMAND_GROUPS 8000
#define _MAX_NUMBER_OF_ARCS 30000
#define _MAX_NUMBER_OF_AGENTS 200000
#define _MAX_NUMBER_OF_ITERATIONS 300
...
map<int, int>pax_id_to_index_map;
map<string, int>arc_to_index_map;

map<int, vector<int>>pax_agent_map;
map<string, vector<int>> arc_agent_map;
...
void g_read_input_data()
{
	...
	if (parser.OpenCSVFile("input_passenger_demand.csv", true))
	{
		...
		while (parser.ReadRecord())  // if this line contains [] mark, then we will also read field headers.
		{
			...
			pax_id_to_index_map.insert({ key_pax_group_id ,i });
	...
	if (parser.OpenCSVFile("input_arc_capacity.csv", true))
	{
		...
		while (parser.ReadRecord())  // if this line contains [] mark, then we will also read field headers.
		{
			...
			arc_to_index_map.insert({ key_arc_cap ,i });
	...
	if (parser.OpenCSVFile("input_pax_agent_dic.csv", true))
	{
		while (parser.ReadRecord())  // if this line contains [] mark, then we will also read field headers.
		{
			int pax_group_id;
			parser.GetValueByFieldName("pax_id", pax_group_id);

			string r_served_agent_group;
			parser.GetValueByFieldName("agent_index", r_served_agent_group);
			vector<int> r_served_agent_group_vec;
			r_served_agent_group_vec = ParseLineToIntegers(r_served_agent_group);
			
			pax_agent_map.insert({ pax_group_id ,r_served_agent_group_vec });
		}
		parser.CloseCSVFile();
	}

	if (parser.OpenCSVFile("input_arc_agent_dic.csv", true))
	{
		int i = 0;
		while (parser.ReadRecord())  // if this line contains [] mark, then we will also read field headers.
		{
			string arc_id;
			parser.GetValueByFieldName("arc_id", arc_id);
			...
			arc_agent_map.insert({ arc_id ,r_served_agent_group_vec });

			if (i % 1000 == 0)
			{
				cout << "generating arc_agent_map index number: " << i << endl;
			}
...
void g_ADMM_Newton()
{
	...
	/*	float sub_cost_pax[_MAX_NUMBER_OF_AGENTS];
		...
		{
			vector<int> m_used_agent_vec;
			m_used_agent_vec = pax_agent_map.at(g_pax_group_id_vector[k]);
			for (int m = 0; m < m_used_agent_vec.size(); m++)
			{
				//sum(k, f(k)*delta(k,p))
				served_times_for_pax[k] = served_times_for_pax[k] + g_agent_vector[m_used_agent_vec[m]].column_flow;
			}
		...
		{
			vector<int> m_used_agent_vec;
			m_used_agent_vec = arc_agent_map.at(g_arc_vector[r]);

			for (int m = 0; m < m_used_agent_vec.size(); m++)
			{
				//x_a*delat(a,arc(i,j,t,s))
				served_times_for_arc[r] = served_times_for_arc[r] + g_agent_vector[m_used_agent_vec[m]].column_flow;
			}
		...
		{
			for (int k = 0; k < g_agent_vector[m].served_pax_group_vector.size(); k++)
			{
				int n = pax_id_to_index_map.at(g_agent_vector[m].served_pax_group_vector[k]);
				//sum(p,delta(k, p)*[sum(k, f(k)*delat(k, p)) - q(p)])
				sub_cost_pax[m] = sub_cost_pax[m] + Subgradient_pickup_pax[n];
				//sum(p,lambda(p)*delat(k,p))
				penalty_subgradient_agent_pax[m] = penalty_subgradient_agent_pax[m] + lambda_pax[n];
			}

			for (int r = 0; r < g_agent_vector[m].column_node_time_arc_vector.size(); r++)
			{
				int n = arc_to_index_map.at(g_agent_vector[m].column_node_time_arc_vector[r]);
				//sum(arc,delta(k, arc)*[sum(k, f(k)*delat(k, arc)) - cap(arc)])
				sub_cost_arc[m] = sub_cost_arc[m] + Subgradient_visit_arc[n] ;
				//sum(arc, lambda(arc)*delta(k, arc))
				penalty_subgradient_agent_arc[m] = penalty_subgradient_agent_arc[m] + lambda_arc[n];
			}
			...
			for (int k = 0; k < g_agent_vector[m].served_pax_group_vector.size(); k++)
			{
				...
				served_times_for_pax[n] += (update_column_flow- previous_column_flow);

				Subgradient_pickup_pax[n] = served_times_for_pax[n] - g_group_demand_vector[n];
			}
			...
			for (int r = 0; r < g_agent_vector[m].column_node_time_arc_vector.size(); r++)
			{
				...
				served_times_for_arc[n] += (update_column_flow - previous_column_flow);
				Subgradient_visit_arc[n] = served_times_for_arc[n] - g_arc_cap_vector[n];
			}
		}
...
void g_Upper_Bound()
{
	...
	{
		for (int j = 0; j < g_agent_vector[i].served_pax_group_vector.size(); j++)
		{
			int n = pax_id_to_index_map.at(g_agent_vector[i].served_pax_group_vector[j]);
			served_times_for_pax[n] += g_agent_vector[i].column_flow ;

			if (served_times_for_pax[n] > g_group_demand_vector[n])
			{
				float served_exceeded_times_pax = served_times_for_pax[n] - g_group_demand_vector[n];
				g_agent_vector[i].column_flow = g_agent_vector[i].column_flow - served_exceeded_times_pax;
				served_times_for_pax[n] = served_times_for_pax[n] - served_exceeded_times_pax;
			}
		}

		float served_exceeded_times_arc=0.0;

		for (int j = 0; j < g_agent_vector[i].column_node_time_arc_vector.size(); j++)
		{
			int n = arc_to_index_map.at(g_agent_vector[i].column_node_time_arc_vector[j]);
			served_times_for_arc[n] += g_agent_vector[i].column_flow ;

			if (served_times_for_arc[n] > g_arc_cap_vector[n])
			{
				served_exceeded_times_arc = served_times_for_arc[n] - g_arc_cap_vector[n];
				g_agent_vector[i].column_flow = g_agent_vector[i].column_flow - served_exceeded_times_arc;
				served_times_for_arc[n] = served_times_for_arc[n] - served_exceeded_times_arc;
			}
		}
		...
		{
			int n = pax_id_to_index_map.at(g_agent_vector[i].served_pax_group_vector[j]);
			served_times_for_pax[n] -= served_exceeded_times_arc;
		}
...
void g_output_GAMS_Parameters()
{
	...
	else
	{
		...
		for (int i = 0; i < g_agent_vector.size(); i++)
		{
			...
			{
				int n = arc_to_index_map.at(g_agent_vector[i].column_node_time_arc_vector[j]);
				std::string str = g_arc_GAMS_format_map[n];
				char *cstr = &str[0u];
		...
		for (int i = 0; i < g_agent_vector.size(); i++)
		{
			...
			{
				int n = pax_id_to_index_map.at(g_agent_vector[i].served_pax_group_vector[j]);
				fprintf(g_pFileGAMS, "%d.%d %d\n", g_agent_vector[i].agent_id, g_pax_group_id_vector[n], 1);
			}
		}
		fprintf(g_pFileGAMS, "/;\n");
	}
...
void g_output_upper_bound_solutions()
{
	...
	{
		vector<int> m_used_agent_vec;
		m_used_agent_vec = arc_agent_map.at(g_arc_vector[j]);

		for (int i = 0; i < m_used_agent_vec.size(); i++)
		{
			g_arc_flow[j] += g_agent_vector[m_used_agent_vec[i]].column_flow;
		}
